...
?>
		<div class="index col-12">
            <div class="card card-lightblue">
				...
				<?php //= __('Cities') ?>	
				<div class="card-body table-responsive p-0 cities">
...
<?php } ?>
								...
								<th class="zip string"><?= $this->Paginator->sort('zip') ?></th>
								<th class="county-short char"><?= $this->Paginator->sort('county_short') ?></th>
...
<?php } ?>
					...
					<?php foreach ($cities as $city): ?>
							<tr row-id="<?= $city->id ?>"<?php if($city->id == $layoutCitiesLastId){ echo ' class="table-tr-last-id"'; } ?>  prefix="<?= $prefix ?>" controller="<?= $controller ?>" action="<?= $action ?>" aria-expanded="true">
								<td class="row-id-anchor" value="<?= $city->id ?>"><a class="anchor" name="<?= $city->id ?>"></a></td><!-- bake-0 -->
<?php if(isset($config['index_show_id']) && $config['index_show_id']){ ?>
								<td class="id integer" name="id" value="<?= $this->Number->format($city->id) ?>"><?= $this->Number->format($city->id) ?></td><!-- bake-3 -->
<?php } ?>
								<td class="name string" name="name" value="<?= $city->name ?>"><?= h($city->name) ?></td><!-- bake-2 -->
								<td class="zip string" name="zip" value="<?= $city->zip ?>"><?= h($city->zip) ?></td><!-- bake-2 -->
								<td class="county-short char" name="county-short" value="<?= $city->county_short ?>"><?= h($city->county_short) ?></td><!-- bake-2 -->
								<td class="longitude string" name="longitude" value="<?= $city->longitude ?>"><?= h($city->longitude) ?></td><!-- bake-2 -->
								<td class="latitude string" name="latitude" value="<?= $city->latitude ?>"><?= h($city->latitude) ?></td><!-- bake-2 -->
<?php if(isset($config['index_show_counts']) && $config['index_show_counts']){ ?>
								<td class="person-count integer" name="person-count" value="<?= $this->Number->format($city->person_count) ?>"><?= $this->Number->format($city->person_count) ?></td><!-- bake-3 -->
<?php } ?>
...
<?php 	if(isset($config['index_enable_view']) && $config['index_enable_view']){ ?>					  
									<?= $this->Html->link('<i class="fas fa-eye"></i>', ['action' => 'view', $city->id], ['escape' => false, 'class' => 'btn btn-sm bg-gradient-warning action-button-view', 'data-bs-tooltip'=>'tooltip', 'data-bs-placement'=>'top', 'title' => __('View this record')]) ?>
<?php 	} ?>
<?php 	if(isset($config['index_enable_edit']) && $config['index_enable_edit']){ ?>					  
									<?= $this->Html->link('<i class="fas fa-edit"></i>', ['action' => 'edit', $city->id], ['escape' => false, 'class' => 'btn btn-sm bg-gradient-success action-button-edit', 'data-bs-tooltip'=>'tooltip', 'data-bs-placement'=>'top', 'title' => __('Edit this record')]) ?>
<?php 	} ?>			
<?php 	if(isset($config['index_enable_delete']) && $config['index_enable_delete']){ ?>					  
									...
									<?= $this->Form->postLink('', ['action' => 'delete', $city->id], ['class'=>'crose-btn hide-postlink action-button-delete']) ?>
									<a href="javascript:;" class="btn btn-sm btn-danger delete postlink-delete" data-bs-tooltip="tooltip" data-bs-placement="top" title="<?= __("Delete this record!") ?>" text="<?= h($city->name) ?>" subText="<?= __("You will not be able to revert this!") ?>" confirmButtonText="<?= __("Yes, delete it!") ?>" cancelButtonText="<?= __("Cancel") ?>"><i class="icon-minus"></i></a>
...
<?php $url = $this->Url->build(['controller' => 'Cities', 'action' => $config['index_db_click_action']]); ?>
		$('tr').dblclick( function(){
<?php /* window.location.href = '/<?= $prefix ?>/cities/<?= $config['index_db_click_action'] ?>/'+$(this).attr('row-id'); */ ?>
...
<?php if(isset($controller)){ ?>
		$(".pagination a[href^='<?= $base ?>/<?= $prefix ?>/cities?sort=']").each(function(){ 
			this.href = this.href.replace("<?= $base ?>/<?= $prefix ?>/cities?sort=", "<?= $base ?>/<?= $prefix ?>/cities?page=1&sort=");
		});
		$(".pagination a[href='<?= $base ?>/<?= $prefix ?>/cities']").each(function(){ 
			this.href = this.href.replace("<?= $base ?>/<?= $prefix ?>/cities", "<?= $base ?>/<?= $prefix ?>/cities?page=1");
		});
<?php } ?>
